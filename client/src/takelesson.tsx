...
function CourseButton({
  ...
  return (
    <button
      ...
      className="w-full text-left px-6 py-4 rounded-lg transition shadow font-medium text-lg bg-white hover:bg-blue-100 border border-blue-500 text-black-500"
    >
      {course.Title}
    </button>
  );
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const withStatus: CourseWithStatus[] = await Promise.all(
